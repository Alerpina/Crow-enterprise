...
<div class="floating_button_sellers">
    ...
    <a href="https://api.whatsapp.com/send?1=pt_BR&phone={!!$gs->whatsapp_number!!}&text="
    ...
    @endif
    @foreach($twhatsapp as $tcategory)
    <a href="#" data-toggle="modal" data-target="{{'#modalSales'.$tcategory->id}}" class=" floating_button_seller">
        <button class="btn btn-secondary mb-0">{{$tcategory->name}}</button>
    </a>
    @endforeach
    <div>
        <a target="_blank" class="floating_button_seller main" id="wppHov"><img
                src="{{asset('assets/front/images/icon/wpp_mini.png')}}"></a>
    </div>
</div>
...
<div>
    <a href="https://api.whatsapp.com/send?1=pt_BR&phone={!!$gs->whatsapp_number!!}&text=" target="_blank">
        <img class="whatsapp-widget" id="whatsapp-widget" style="cursor:pointer"
            src="{{asset('assets/front/images/icon/wpp_mini.png')}}">
    </a>
</div>
