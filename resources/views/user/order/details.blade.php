                    ...
                    <div class="order-details">
                        ...
                        <div class="view-order-page">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <p>{{ __("Payment Status") }}:
                                            {!! $order->payment_status == 'Pending' ? "<span
                                                class='badge badge-danger'>". __("Unpaid") ."</span>":"<span
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        ...
                                        <address>
                                            ...
                                            : $order->shipping_address_number}} <br>
                                            {{$order->shipping_complement == null ? $order->customer_complement :
                                            $order->shipping_complement}}<br>
                                            {{$order->shipping_district == null ? $order->customer_district :
                                            $order->shipping_district}}<br>
                                            {{$order->shipping_city == null ? $order->customer_city :
                                            $order->shipping_city}} - {{$order->shipping_state == null ?
                                            $order->customer_state : $order->shipping_state}}<br>
                                            {{$order->shipping_country == null ? $order->customer_country :
                                            $order->shipping_country}}<br>
                                            {{$order->shipping_zip == null ? $order->customer_zip :
                                            $order->shipping_zip}}
                                        </address>
                                        @else
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <p>
                                            ...
                                            {{ $order->shipping_type }}: {{$order->currency_sign}}{{
                                            number_format($order->shipping_cost * $order->currency_value,
                                            $order_curr->decimal_digits,
                                            $order_curr->decimal_separator,$order_curr->thousands_separator) }}<br>
                                            @if($order->puntoentrega != null)
                                            {{ __("Delivery Point:") }} {{$order->puntoentrega}}<br>
                                            @endif
                                            {{ $order->packing_type }}: {{$order->currency_sign}}{{
                                            number_format($order->packing_cost * $order->currency_value,
                                            $order_curr->decimal_digits,
                                            $order_curr->decimal_separator,$order_curr->thousands_separator) }}<br>
                                        </p>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <p>
                                            ...
                                            {{ __('Discount:') }} {{$order->currency_sign}}{{
                                            number_format($order->coupon_discount * $order->currency_value,
                                            $order_curr->decimal_digits,
                                            $order_curr->decimal_separator,$order_curr->thousands_separator) }}<br>
                                            @endif
                                            {{ __("Paid Amount:") }} {{$order->currency_sign}}{{
                                            number_format($order->pay_amount * $order->currency_value,
                                            $order_curr->decimal_digits,
                                            $order_curr->decimal_separator,$order_curr->thousands_separator) }}<br>
                                            @if($order->pay42_exchange_rate)
                                        <p style="font-size: 20px">
                                            {{ $first_curr->sign.' '.__('Total to be paid:') }}
                                            {{$first_curr->sign}}{{ number_format($order->pay42_total,
                                            $first_curr->decimal_digits,
                                            $first_curr->decimal_separator,$first_curr->thousands_separator) }}<br>
                                            {{__('Pay42 exchange rate: ').$order->pay42_exchange_rate}}
                                        </p>
                                        @else
                                        {{ $first_curr->sign.' '.__('Total:') }} {{$first_curr->sign}}{{
                                        number_format($order->pay_amount, $first_curr->decimal_digits,
                                        $first_curr->decimal_separator,$first_curr->thousands_separator) }}<br>
                                        </p>
                                        @endif
                                        ...
                                        {{$order->method}} {{ __("Charge ID:") }} <p>{{$order->charge_id}}</p>
                                        @endif
                                        {{$order->method}} {{ __("Transaction ID:") }} <p id="ttn"> {{$order->txnid}}
                                        </p>
                            ...
                            @if(now() < $order->pay42_due_date)
                                ...
                                <div style="display:flex; justify-content:center;">
                                    <p class="pix"> PIX QR CODE {{__('Expire in: ')}} {{$order->pay42_due_date.' UTC'}}
                                    </p>
                                </div>
                                @else
                                <div style="display:flex; justify-content:center;">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td width="40%">
                                                    ...
                                                    <b>{{ __("Custom Name") }}</b>: {{ $product['customizable_name']
                                                    }}<br>
                                                    @endif
                                                    @endif
                                                    @if(env("ENABLE_CUSTOM_PRODUCT_NUMBER"))
                                                    @if(!empty($product['customizable_number']))
                                                    <b>{{ __("Custom Number") }}</b>: {{ $product['customizable_number']
                                                    }}<br>
                                                    @endif
                                                    @endif
                                                    @if(env("ENABLE_CUSTOM_PRODUCT"))
                                                ...
                                                <td style="text-align: end;">
                                                    ...
                                                    $order->currency_value, $order_curr->decimal_digits,
                                                    $order_curr->decimal_separator,$order_curr->thousands_separator)}}<br><small>{{$first_curr->sign}}{{
                                                ...
                                                <td style="text-align: end;">
                                                    ...
                                                    $order->currency_value, $order_curr->decimal_digits,
                                                    $order_curr->decimal_separator,$order_curr->thousands_separator)}}<br><small>{{$first_curr->sign}}{{
                                                        number_format($product['price'], $first_curr->decimal_digits,
                                                        $first_curr->decimal_separator,$first_curr->thousands_separator)
                                                        }}</small>
                                                </td>
