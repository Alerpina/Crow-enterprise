                                        ...
                                        <div class="row reorder-xs">
                                            <div class="col-lg-8 col-md-5 col-sm-5 col-xs-12">
                                                <div class="product-header-title">
                                                    <h2>{{ __("Order#") }} {{$order->order_number}} [{{__($order->status)}}]</h2>
                                        </div>   
                                    </div>
                                        <div class="row">
                                            <div class="col-md-10">
                                                            ...
                                                            <div class="row">
                                                                <div class="col-md-6">
                                                                    ...
                                                                    <address>
                                                                        ...
                                                                        {{ __('Number:') }} {{$order->customer_address_number}} {{$order->customer_complement}}<br>
                                                                ...
                                                                <div class="col-md-6">
                                                                    ...
                                                                    <p>{{ __("Paid Amount:") }} {{$order->currency_sign}}{{ number_format($order->pay_amount * $order->currency_value , $order_curr->decimal_digits, $order_curr->decimal_separator,$order_curr->thousands_separator) }}</p>
                                                                    <p>{{ $first_curr->sign.' '.__('Total:') }} {{$first_curr->sign}}{{ number_format($order->pay_amount, $first_curr->decimal_digits, $first_curr->decimal_separator,$first_curr->thousands_separator) }}</p>
...
@else
                                            ...
                                            <p>{{ __('Discount:') }} {{$order->currency_sign}}{{ number_format($order->coupon_discount * $order->currency_value , $order_curr->decimal_digits, $order_curr->decimal_separator,$order_curr->thousands_separator) }}</p>
                                            @endif
                                                                    <p>{{ __("Paid Amount:") }} {{$order->currency_sign}}{{ number_format($order->pay_amount * $order->currency_value , $order_curr->decimal_digits, $order_curr->decimal_separator,$order_curr->thousands_separator) }}</p>
                                                                    <p>{{ $first_curr->sign.' '.__('Total:') }} {{$first_curr->sign}}{{ number_format($order->pay_amount, $first_curr->decimal_digits, $first_curr->decimal_separator, $first_curr->thousands_separator) }}</p>
                                                                    <p>{{ __("Payment Method:") }} {{$order->method}}</p>
                                                                    ...
                                                                    @if($order->method=="Bank Deposit")
                                                                        <div style="margin-top: 10px; font-size: small;">
                                                                            ...
                                                                            <ul class="list-group" style="margin-top: 10px">
                                                                                <li class="list-group-item" style="padding: 5px;">{{strtoupper($bank_account->name)}}</li>
                ...
                {{$order->shipping_city == null ? $order->customer_city : $order->shipping_city}} - {{$order->shipping_state == null ? $order->customer_state : $order->shipping_state}}<br>
                ...
                {{$order->shipping_zip == null ? $order->customer_zip : $order->shipping_zip}}
                                                                    ...
                                                                    <p>
                                                                    {{ $order->shipping_type }}: {{$order->currency_sign}}{{ number_format($order->shipping_cost * $order->currency_value , $order_curr->decimal_digits, $order_curr->decimal_separator,$order_curr->thousands_separator) }}<br>
                                                                    
                                                                    {{ $order->packing_type }}: {{$order->currency_sign}}{{ number_format($order->packing_cost * $order->currency_value , $order_curr->decimal_digits, $order_curr->decimal_separator,$order_curr->thousands_separator) }}<br>  
...
@endif
                            @if($order->is_qrcode)
                                @if(now() < $order->pay42_due_date)
                                    ...
                                    <div style="display:flex; justify-content:center;">
                                        <p class="pix"> PIX QR CODE {{__('Expire in: ')}} {{$order->pay42_due_date.' UTC'}}</p>
                                    </div>
                                @else
                                    <div style="display:flex; justify-content:center;">
                                            ...
                                            <td>
                                                ...
                                                @endif
                                                    ...
                                                    @foreach( array_combine(explode(',', $product['keys']), explode('~', $product['values']))  as $key => $value)
                                                        ...
                                                        <b>{{ App\Models\Attribute::where('input_name', $key)->first()->name }} : </b> {{ $value }} <br>
                                                    @endforeach

                                                    @endif

                                                  </td>
                                            <td style="text-align: end;">{{$order->currency_sign}}{{number_format($product['item']['price'] * $order->currency_value , $order_curr->decimal_digits, $order_curr->decimal_separator,$order_curr->thousands_separator)}}
                                            <br><small>{{$first_curr->sign}}{{ number_format($product['item']['price'], $first_curr->decimal_digits,$first_curr->decimal_separator,$first_curr->thousands_separator) }}</small></td>
                                            <td style="text-align: end;">{{$order->currency_sign}}{{number_format($product['price'] * $order->currency_value , $order_curr->decimal_digits, $order_curr->decimal_separator,$order_curr->thousands_separator)}}
                                            <br><small>{{$first_curr->sign}}{{ number_format($product['price'], $first_curr->decimal_digits, $first_curr->decimal_separator,$first_curr->thousands_separator) }}</small></td>
