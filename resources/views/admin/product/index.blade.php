...
<div class="content-area">
	...
	$(document).on('click', '.fasteditbtn', function() {
		...
		$('#fast_edit_modal .modal-content .modal-body').html('').load($(this).attr('data-href'), function(response, status,
			xhr) {
			if (status == "success") {
				if (admin_loader == 1) {
					$('.submit-loader').hide();
	...
	$(document).on('click', '.bulkeditbtn', function() {
        ...
        $('#bulk_edit_modal .modal-content .modal-body').html('').load($(this).attr('data-href'), function(response, status,
			xhr) {
			if (status == "success") {
				if (admin_loader == 1) {
					$('.submit-loader').hide();
        ...
        drawCallback: function(settings) {
            ...
            */
            ...
            $(".checkboxFeatured").on('click', function(){
                ...
                $.ajax({
                    type: 'GET',
                    url: '{{ url("admin/products/featured") }}' + '/' + id + '/' + featuredNovo
                });
            });
        ...
        */
        ...
        $(document).on('click', 'a', function(e){
            ...
            if(!(link.attr("data-href") || link.attr("href").indexOf("#") > -1 || link.attr("href").indexOf("javascript") > -1 || link.attr("href").indexOf(y[1]) > -1)){
                sessionStorage.setItem("CurrentPage", 0);
                sessionStorage.setItem('SelectedProductFilter', $("#product_filters").find("option:first").val());
                sessionStorage.setItem('SelectedBrandFilter', $("#brand_filters").find("option:first").val());
                sessionStorage.setItem('SelectedCategoryFilter', $("#category_filters").find("option:first").val());
                sessionStorage.setItem('SelectedStoreFilter', $("#store_filters").find("option:first").val());
                table.state.clear();
            }
        });
    });
