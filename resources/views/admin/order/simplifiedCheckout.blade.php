                    ...
                    <li>
                        <div class="action-list godropdown">
                            <select id="order_simplified_filters" class="process select go-dropdown-toggle">
                                ...
                                <option value="{{ route('admin-order-simplified-datatables', $filter) }}">{{$name}}
                                </option>
                                @endforeach
                            </select>
                        </div>
...
<script>
    $(document).ready(function(){
        ...
        if(sessionStorage.getItem("CurrentPage") == undefined){
            sessionStorage.setItem("CurrentPage", 0);
            sessionStorage.setItem('SelectedOrderSimplifiedFilter', $("#order_simplified_filters").find("option:first").val());
        }
    });
    $("#order_simplified_filters").on('change', function() {
    ...
    $(document).on('click', 'a', function(e){
        ...
        if(link.attr("href").indexOf("orders") > 1){
            sessionStorage.setItem("CurrentPage", 0);
            sessionStorage.setItem('SelectedSimplifiedOrderFilter', $("#order_simplified_filters").find("option:first").val());
            table.state.clear();
        }
    });
</script>
