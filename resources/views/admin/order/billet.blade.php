                ...
                <div class="input-form">
                    <div class="table-responsive" style="overflow-x: hidden;">
                        <h4>{{ __("Order Details") }}</h5>
                            ...
                            <h6>{{ __("Total") . ": " . $order->currency_sign}}{{ number_format($order->pay_amount *
                                $order->currency_value , $order_curr->decimal_digits,
                                $order_curr->decimal_separator,$order_curr->thousands_separator) }}</h6>
                            @if($billet["status_request"]["status"] == "pending")<span class="badge badge-secondary">{{
                                __("Billet Status") . ": ". __("Pending") }}</span>
                            @elseif($billet["status_request"]["status"] == "canceled")<span
                                class="badge badge-danger">{{ __("Billet Status") . ": ". __("Canceled") }}</span>
                            @elseif($billet["status_request"]["status"] == "paid")<span class="badge badge-success">{{
                                __("Billet Status") . ": ". __("Paid") }}</span>
                            @endif
                    </div>
                </div>
            </div>
            ...
            <div class="col-lg-12 text-center">
                <div class="input-form">
                    <a class="btn btn-success" target="_blank"
                        href="https://web.whatsapp.com/send?phone={{ $ddi }}{{ $order->customer_phone }}&text={{ $msg }}">{{
                        __("Re-send Billet via WhatsApp")}}</a>
                </div>
            </div>
        </div>
        @endif
