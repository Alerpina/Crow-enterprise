        ...
        <div class="special-box">
          ...
          <form class="uplogo-form" id="geniusform" action="{{ route('admin-gs-update') }}" method="POST"
            ...
            <div class="currrent-logo">
              ...
              <img
                src="{{ $admstore->loader ? asset('assets/images/'.$admstore->loader):asset('assets/images/noimage.png')}}"
                alt="">
            </div>
              ...
              <div class="row">
                    ...
                    <div class="action-list">
                      <select
                        ...
                        <option data-val="1" value="{{route('admin-gs-isloader',1)}}"
                          {{ $admstore->is_loader == 1 ? 'selected' : '' }}>{{ __('Activated') }}</option>
                        <option data-val="0" value="{{route('admin-gs-isloader',0)}}"
                          {{ $admstore->is_loader == 0 ? 'selected' : '' }}>{{ __('Deactivated') }}</option>
                      </select>
                    </div>
                  </div>
                </div>
                ...
                <div class="col-xl-9">
                  <div class="input-form input-form-center" style="overflow:hidden;">
                    <h4 class="heading">
                      <i class="icofont-question-circle" data-toggle="tooltip" style="display: inline-block " data-placement="top" title="{{ __('Loader animation') }}"></i> {{ __('Set New Loader') }} :
                    </h4>
        ...
        <div class="special-box">
          ...
          <form class="uplogo-form" id="geniusform" action="{{ route('admin-gs-update') }}" method="POST"
            ...
            <div class="currrent-logo">
              ...
              <img
                src="{{ $admstore->admin_loader ? asset('assets/images/'.$admstore->admin_loader):asset('assets/images/noimage.png')}}"
                alt="">
            </div>
                    ...
                    <div class="action-list">
                      <select
                        ...
                        <option data-val="1" value="{{ route('admin-gs-is-admin-loader',1) }}"
                          {{ $admstore->is_admin_loader == 1 ? 'selected' : '' }}>{{ __('Activated') }}</option>
                        <option data-val="0" value="{{ route('admin-gs-is-admin-loader',0) }}"
                          {{ $admstore->is_admin_loader == 0 ? 'selected' : '' }}>{{ __('Deactivated') }}</option>
                      </select>
                    </div>
                  </div>
                </div>
