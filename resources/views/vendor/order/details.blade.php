                    ...
                    <div class="table-responsive-sm">
                        <table class="table">
                            <tbody>
                                ...
                                <tr>
                                    <th width="45%">{{ Shipping::where('price','=',$price)->first()->title }}</th>
                                    <td width="10%">:</td>
                                    <td width="45%">{{ $order->currency_sign }}{{ round($order->shipping_cost, 2) }}
                                    </td>
                                </tr>
                                @endif
                                @endif
                                ...
                                <tr>
                                    <th width="45%">{{ Package::where('price','=',$pprice)->first()->title }}</th>
                                    <td width="10%">:</td>
                                ...
                                <tr>
                                    ...
                                    $price =
                                    number_format($order->vendororders()->where('user_id','=',$user->id)->sum('price'),2);
                                    ...
                                    <td width="45%">{{$order->currency_sign}}{{ round($price * $order->currency_value ,
                                        2) }}</td>
                                </tr>
                                <tr>
                                ...
                                <tr>
                                    ...
                                    <th width="10%">:</th>
                                    <td width="45%">{!! $order->payment_status == 'Pending' ? "<span
                                            class='badge badge-danger'>". __("Unpaid") ."</span>":"<span
                                            class='badge badge-success'>". __("Paid") ."</span>" !!}</td>
                                </tr>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <span class="badge badge-warning">{{__(ucwords($user->status))}}</span>
                                        @elseif($user->status == 'processing')
                                        <span class="badge badge-info">{{__(ucwords($user->status))}}</span>
                                        @elseif($user->status == 'on delivery')
                                        <span class="badge badge-primary">{{__(ucwords($user->status))}}</span>
                                        @elseif($user->status == 'completed')
                                        <span class="badge badge-success">{{__(ucwords($user->status))}}</span>
                                        @elseif($user->status == 'declined')
                                        <span class="badge badge-danger">{{__(ucwords($user->status))}}</span>
                                        @endif
                                    ...
                                    <td>
                                        ...
                                        <a target="_blank"
                                            ...
                                            $product['item']['name']}}</a>
                                        @else
                                        <a href="javascript:;">{{mb_strlen($product['item']['name'],'utf-8') > 30 ?
                                        ...
                                        <a href="javascript:;" data-toggle="modal" data-target="#confirm-delete"
                                            class="btn btn-info product-btn" id="license" style="padding: 5px 12px;"><i
                                                class="fa fa-eye"></i> View License</a>
                                        @endif
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p class="text-center">{{ __("The Licenes Key is") }} : <span id="key"></span> <a href="javascript:;"
                        id="license-edit">{{ __("License Key") }}</a><a href="javascript:;" id="license-cancel"
                ...
                <form method="POST" action="{{route('vendor-order-license',$order->order_number)}}" id="edit-license"
                    ...
                    <div class="form-group text-center">
                        <input type="text" name="{{ __(" Save License") }}" placeholder="{{ __(" Enter New License Key")
                                        ...
                                        <ul>
                                            <li>
                                                <input type="email" class="input-field eml-val" id="eml" name="to"
                                                    placeholder="{{ __(" Email Address") }} *" value="" required="">
                                            </li>
                                            <li>
