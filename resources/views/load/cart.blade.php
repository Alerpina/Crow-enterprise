...
<div class="dropdownmenu-wrapper">
	...
	<ul class="dropdown-cart-products">
		...
		@php
			$custom_item_id = $product['item']['id'].$product['size'].$product['material'].$product['color'].$product['customizable_gallery'].$product['customizable_name'].$product['customizable_number'].$product['customizable_logo'].str_replace(str_split(' ,'),'',$product['values']);
			$custom_item_id = str_replace( array( '\'', '"', ',', '.', ' ', ';', '<', '>' ), '', $custom_item_id);
		@endphp

		<li
			class="product cremove{{ str_replace('~','',$custom_item_id) }}">
				...
				<div class="content">
					...
					<span class="cart-product-info">
						<span class="cart-product-qty"
							id="cqt{{ $custom_item_id }}">{{$product['qty']}}</span><span>{{ $product['item']['measure'] }}</span>
						x <span
							id="prct{{ $custom_item_id }}">{{ App\Models\Product::convertPrice($product['item']['price']) }}</span>
					</span>
				</div>
			...
			<figure class="product-image-container">
				...
				<div class="cart-remove"
					...
					data-href="{{ route('product.cart.remove',$custom_item_id) }}"
	...
	<div class="dropdown-cart-action">
		<a href="#" class="mybtn1" data-toggle="modal"
