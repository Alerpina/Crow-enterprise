...
class GalleryController extends Controller
{
    ...
    public function show()
    {
        ...
        if($this->storeSettings->ftp_folder) {
            $ftp_path = public_path('assets/images/ftp/' . $this->storeSettings->ftp_folder . $prod->ref_code_int . '/');
            $ftp_gallery=[];
            if(File::exists($ftp_path)){
                ...
                foreach($files as $file){
                    ...
                    if(in_array($file_extension, $extensions) === true){
                        $ftp_gallery[] = asset('assets/images/ftp/' . $this->storeSettings->ftp_folder . $prod->ref_code_int . '/' . $file);
                    }
                }
                $data[0] = 1;
                $data[2] = $ftp_gallery;
    ...
    { 
        ...
        $img->save(public_path().'/assets/images/galleries360/'.$thumbnail);
    ...
    public function destroy360()
    {
        ...
        $gal = Gallery360::findOrFail($id);
            if (file_exists(public_path().'/assets/images/galleries360/'.$gal->photo)) {
