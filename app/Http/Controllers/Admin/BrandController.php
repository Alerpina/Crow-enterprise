...
class BrandController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($banner = $request->file('banner'))
        {
            ...
            $banner->move('assets/images/brands/banners',$name);
            $input['banner'] = $name;
        }
        if ($request->partner == ""){
            $input['partner'] = 0;
        ...
        if(!empty($input['image'])){
            // Set Thumbnail
            $img = Image::make(public_path().'/assets/images/brands/'.$input['image'])->resize(null, 285, function($constraint){
                $constraint->aspectRatio();
            });
            $img->save(public_path().'/assets/images/thumbnails/'.$input['image']);
    ...
    public function update(Request $request, $id)
    {
        ...
        $input = $request->all();
            if ($file = $request->file('image'))
            {
                ...
                if($data->image != null)
                {
                    if (file_exists(public_path().'/assets/images/brands/'.$data->image) && !empty($data->image)) {
            ...
            if ($banner = $request->file('banner')) {
                ...
                if($data->banner != null)
                {
                    if (file_exists(public_path().'/assets/images/brands/banners/'.$data->banner) && !empty($data->banner)) {
                        unlink(public_path().'/assets/images/brands/banners/'.$data->banner);
                    }
                }
                $input['banner'] = $name;
            }
    ...
    public function destroy($id)
    {
        ...
        if (file_exists(public_path().'/assets/images/brands/'.$data->image) && !empty($data->image)) {
            unlink(public_path().'/assets/images/brands/'.$data->image); 
            //If Thumbnail Exist
            if (file_exists(public_path().'/assets/images/thumbnails/'.$data->thumbnail) && !empty($data->thumbnail)) {
    ...
    public function generateThumbnails(){
        ...
        foreach($brands as $brand){
            ...
            if (!file_exists($thumb_path.$brand->image)) {
                ...
                if($brand->update()){
                    $img_dir = public_path().'/assets/images/brands/'.$brand->image;
    ...
    public function deleteImage(Request $request)
    {
        ...
        $path = public_path().'/assets/images/brands/'.$data->{$request->target};
        $thumbnail_path = public_path().'/assets/images/thumbnails/';
