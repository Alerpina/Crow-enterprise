...
class Helper
{
    ...
    public static function updateAllLanguages($overwrite_all = false, $overwrite_keys = array())
    {
        ...
        foreach ($languages as $data) {
            if ($overwrite_all) {
                ...
                if (file_exists(resource_path("lang") . '/base_' . $data->locale . '.json')) {
                    ...
                    foreach ($overwrite_keys as $overwrite_key) {
                        if (array_key_exists($overwrite_key, $langJsonBase)) {
                            $langJson[$overwrite_key] = $langJsonBase[$overwrite_key];
                        }
                    }
                    $mydata = json_encode($langJson);
                    file_put_contents(resource_path("lang") . '/' . $data->file, $mydata);
                }
            }
        }
    }
    public static function updateAllAdminLanguages($overwrite_all = false, $overwrite_keys = array())
    {
        $languages = AdminLanguage::orderBy('id')->get();
        foreach ($languages as $data) {
            $langJsonBase = array();
            $langJson = array();
            if ($overwrite_all) {
